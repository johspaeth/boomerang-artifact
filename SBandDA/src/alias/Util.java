...
public class Util {
  ...
  private static Map<SootMethod, ReachableMethods> reachables =
      new HashMap<SootMethod, ReachableMethods>();
  ...
  public static boolean REMOVE_STRING_BUFFER = false;

  public static boolean TWEAK_BODY = false;
  ...
  static {
    ...
    USE_CACHE = false; // !("0".equals(System.getProperty("UseCache")));
    ...
    System.out.println("[PARAMETER] SPA_BUDGET_NODES: " + SPA_BUDGET_NODES + ", SUMM_RATIO: "
        + SUMM_RATIO + ", TIME_BUDGET: " + TIME_BUDGET);
  }

  public static boolean traditionalMayAlias(Local var1, SootMethod m1, Local var2, SootMethod m2,
  ...
  public static boolean mayAlias(Local l1, DefinitionStmt s1, SootMethod m1, Local l2,
      DefinitionStmt s2, SootMethod m2, MayAliasAnalysis maa) {

    aliasStart = System.currentTimeMillis();
    boolean res = true;
    try {
      res = maa.mayAlias(l1, s1, m1, l2, s2, m2);
    } catch (OutOfMemoryError e) {
      System.out.println("[6e9waprU." + Util.BENCH_NAME + "." + Util.MAY_ALIAS
          + (Util.USE_SUMMARY ? ".summ" : ".nosumm") + "] OutOfMemory");
      System.exit(0);
    }
    long totalTime = (System.currentTimeMillis() - aliasStart);
    mayAliasTime += totalTime;
    return res;
  }
  ...
  public static boolean isOutOfBudget() {
    ...
    boolean isTimeout = (System.currentTimeMillis() - aliasStart) > Util.TIME_BUDGET;
    if (isTimeout) {
      System.out.println("TIMEOUT");
      MayAliasAnalysis.queryCrashed = true;
    }
    return isTimeout; // 100ms
  }
  ...
  public static void tweakBody(SootMethod m) {
    ...
    while (Util.TWEAK_BODY && stmtIter.hasNext()) {
      ...
      if (stmt.containsInvokeExpr()) {
         ...
         * arraycopy(src, ..., tgt, ...)
         * 
         * s1: temp = src[i]; s2: tgt[i] = temp;
         */
        if (sig
            .equals("<java.lang.System: void arraycopy(java.lang.Object,int,java.lang.Object,int,int)>")) {
          ...
          AssignStmt s1 =
              Jimple.v().newAssignStmt(temp, Jimple.v().newArrayRef(src, IntConstant.v(0)));
          AssignStmt s2 =
              Jimple.v().newAssignStmt(Jimple.v().newArrayRef(tgt, IntConstant.v(0)), temp);

          units.insertAfter(s1, stmt);
          units.insertAfter(s2, s1);
        } else if (Util.REMOVE_STRING_BUFFER && stmt instanceof AssignStmt
  ...
  public static boolean inLibrary(SootMethod m) {
    ...
    return p.startsWith("java.") || p.startsWith("sun.") || p.startsWith("sunw.")
        || p.startsWith("javax.") || p.startsWith("org.") || p.startsWith("com.");
  }

  public static boolean isVisited(LinkedList<CtxPair> ctxSumm, LinkedList<NumberedFldPair> fldSumm,
      AbstractSPGEdge edge, int ctxHash) {
  ...
  public static boolean isVisitedOld(LinkedList<CtxPair> ctxSumm,
      LinkedList<NumberedFldPair> fldSumm, AbstractSPGEdge edge, int ctxHash) {
    if (edge instanceof FieldPTEdge) {
      for (NumberedFldPair p : fldSumm) {
        if (p.getEdge() == edge && p.getCtxHash() == ctxHash) {
          return true;
  ...
  public static boolean sootFieldEquals(SootField f1, SootField f2) {
    if (f1 == f2)
      return true; // TODO: for ArrayElementField, might be wrong
    try {
  ...
  /*
   * Determines whether we can reach n2 starting from n1 with call graph constraints only.
   */
  ...
  public static boolean subclassOf(SootClass sc1, SootClass sc2) {
    if (sc1.equals(sc2))
      return true;
    if (sc1.getName().equals("java.lang.Object"))
      return false;

    if (subclassOf(sc1.getSuperclass(), sc2))
      return true;

    for (SootClass sc : sc1.getInterfaces()) {
      if (subclassOf(sc, sc2))
        return true;
    }
  ...
  public static HashSet<AbstractAllocNode> getReachables(ReachableMethods mtds,
      Collection<AbstractAllocNode> objs) {
