...
public interface IInfoflow {
  ...
  /**
   * Enumeration containing the callgraph algorithms supported for the use with the data flow
   * tracker
   */
  ...
  public enum AliasingAlgorithm {
     ...
     */
    PtsBased, Boomerang, BoomerangAC, SB, DA
  }
  ...
  public enum CodeEliminationMode {
    ...
    NoCodeElimination,
    /**
     * Perform an inter-procedural constant propagation and folding and then remove all code that is
     * unreachable
     */
    PropagateConstants,
    /**
     * In addition to the inter-procedural constant propagation and folding, also remove live code
     * that cannot potentially influence the outcome of the taint analysis
     */
   ...
   * 
   * @param wrapper The taint wrapper object that decides on how information is propagated through
   *        black-box methods
   */
  public void setTaintWrapper(ITaintPropagationWrapper wrapper);

  /**
   * Gets the taint wrapper for deciding on taint propagation through black-box methods
   * 
   * @return The taint wrapper object that decides on how information is propagated through
   *         black-box methods
   */
   ...
   * 
   * @param stopAfterFirstFlow True if the analysis shall stop after the first flow has been found,
   *        otherwise false.
   */
  ...
  /**
   * Computes the information flow on a list of entry point methods. This list is used to construct
   * an artificial main method following the Android life cycle for all methods that are detected to
   * be part of Android's application infrastructure (e.g. android.app.Activity.onCreate)
   * 
   ...
   * @param entryPointCreator the entry point creator to use for generating the dummy main method
  ...
  /**
   * Computes the information flow on a single method. This method is directly taken as the entry
   * point into the program, even if it is an instance method.
   * 
   * @param appPath The path containing the client program's files
   * @param libPath the path to the main folder of the (unpacked) library class files
  ...
  /**
   * default: inspectSources is set to true, this means sources are analyzed as well. If
   * inspectSources is set to false, then the analysis does not propagate values into the source
   * method.
   * 
  ...
  /**
   * default: inspectSinks is set to true, this means sinks are analyzed as well. If inspectSinks is
   * set to false, then the analysis does not propagate values into the sink method.
   * 
   ...
   * 
   * @param enableStaticFields True if assignments to static fields shall be considered, otherwise
   *        false
   */
   ...
   * 
   * @param enableExceptions True if taints associated with exceptions shall be tracked over
  ...
  /**
   * Sets the maximum number of threads to be used by the solver. A value of -1 indicates an
   * unlimited number of threads, i.e., there will be as many threads as there are CPU cores on the
   * machine.
   * 
   * @param threadNum The maximum number of threads to be used by the solver, or -1 for an unlimited
   *        number of threads.
   */
  ...
  /**
   * Sets whether flows starting or ending in system packages such as Android's support library
   * shall be ignored.
   * 
  ...
  /**
   * Sets whether and how FlowDroid shall eliminate irrelevant code before running the taint
   * propagation
   * 
