...
/**
 * We are subclassing the JimpleIFDSSolver because we need the same executor for both the forward
 * and the backward analysis Also we need to be able to insert edges containing new taint
 * information
 * 
 */
public class InfoflowSolver extends
    PathTrackingIFDSSolver<Unit, Abstraction, SootMethod, BiDiInterproceduralCFG<Unit, SootMethod>>
  ...
  public boolean processEdge(PathEdge<Unit, Abstraction> edge) {
    ...
    // to <n,d2>.
    if (!jumpFn.forwardLookup(edge.factAtSource(), edge.getTarget()).containsKey(
        edge.factAtTarget())) {
  ...
  public void injectContext(IInfoflowSolver otherSolver, SootMethod callee, Abstraction d3,
    ...
    synchronized (solver.jumpFn) {
      otherAbstractions =
          new HashSet<Abstraction>(solver.jumpFn.reverseLookup(callSite, d2).keySet());
    }
  ...
  @Override
  protected Set<Abstraction> computeReturnFlowFunction(FlowFunction<Abstraction> retFunction,
      Abstraction d1, Abstraction d2, Unit callSite, Set<Abstraction> callerSideDs) {
  ...
  @Override
  protected Set<Abstraction> computeNormalFlowFunction(FlowFunction<Abstraction> flowFunction,
  ...
  @Override
  protected Set<Abstraction> computeCallFlowFunction(FlowFunction<Abstraction> flowFunction,
      Abstraction d1, Abstraction d2) {
  ...
  protected void propagate(Abstraction sourceVal, Unit target, Abstraction targetVal,
    ...
    /*
     * for (Abstraction abs : new HashSet<Abstraction>(jumpFn.forwardLookup(sourceVal,
     * target).keySet())) if (abs != targetVal) { if (abs.entails(targetVal)) { noProp = true;
     * break; } if (targetVal.entails(abs)) { jumpFn.removeFunction(sourceVal, target, abs); } }
     */
    if (!noProp)
  ...
  /**
   * Cleans up some unused memory. Results will still be available afterwards, but no intermediate
   * computation values.
   */
  public void cleanup() {
    this.jumpFn.clear();
