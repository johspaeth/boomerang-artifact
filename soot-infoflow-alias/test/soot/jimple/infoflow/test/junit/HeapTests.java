...
public class HeapTests extends JUnitTests {
  ...
  @Test(timeout = 300000)
  public void indirectAlias() {
    Infoflow infoflow = initInfoflow();
    List<String> epoints = new ArrayList<String>();
    epoints.add("<soot.jimple.infoflow.test.HeapTestCode: void indirectAlias()>");
    infoflow.computeInfoflow(appPath, libPath, epoints, sources, sinks);
    checkInfoflow(infoflow, 1);
  }
    ...
    infoflow.setTaintWrapper(new AbstractTaintWrapper() {
      ...
      public boolean isExclusiveInternal(Stmt stmt, AccessPath taintedPath) {
        return stmt.containsInvokeExpr()
            && (stmt.getInvokeExpr().getMethod().getName().equals("foo2") || stmt.getInvokeExpr()
      ...
      public Set<AccessPath> getTaintsForMethodInternal(Stmt stmt, AccessPath taintedPath) {
        ...
        if (stmt.getInvokeExpr().getMethod().getName().equals("foo2")) {
          ...
          if (taintedPath.getPlainValue() == iinv.getArg(0)) {
            ...
            AccessPath ap =
                new AccessPath(iinv.getBase(), new SootField[] {rt.getSootClass().getFieldByName(
                    "b1")}, true);
            res.add(ap);
          }
          ...
          if (taintedPath.getPlainValue() == iinv.getArg(0)) {
            ...
            AccessPath ap =
                new AccessPath(def.getLeftOp(), Scene.v()
